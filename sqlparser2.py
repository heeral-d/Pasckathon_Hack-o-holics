...
def parser(database):
    ...
    import json
    import pickle
    with open('static/table_attributes.json', 'w') as f:
        json.dump(table_attributes, f)
    with open('static/tables_pk.json', 'w') as f:
        json.dump(tables_pk, f)
    # elsewhere...
    pickle.dump( tables_relation, open( "static/tables_relation.p", "wb" ))
    pickle.dump(tables,open("static/tables.p","wb"))
